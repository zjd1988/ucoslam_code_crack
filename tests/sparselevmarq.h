...
public:
        ...
        struct Params{
            Params(){}
            Params(int _maxIters,T _minError,T _min_step_error_diff=0,T _tau=1 ,T _der_epsilon=1e-3){
                maxIters=_maxIters;
                minError=_minError;
...
bool SparseLevMarq<T>::step( F_z_x f_z_x, F_z_J f_J)throw (std::exception){
    ...
    if (_params.verbose) std::cout<<std::setprecision(5) <<"Curr Error="<<currErr<<" AErr(prev-curr)="<<prevErr-currErr<<" gain="<<gain<<" dumping factor="<<mu<<std::endl;
...
if( _stopFunction){
    ...
    for ( int i = 0; i < _params.maxIters && !mustExit; i++ ) {
        ...
        if ( currErr<_params.minError ) mustExit=1;
         if( fabs( prevErr -currErr)<=_params.min_step_error_diff  || !isStepAccepted) mustExit=2;
